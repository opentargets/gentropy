...
class TestLocusToGeneTrainer:
    ...
    def test_cross_validate(
        ...
        param_grid = (
            ParamGridBuilder().addGrid(model._estimator.regParam, [0.1, 0.01]).build()
        )
        best_model = LocusToGeneTrainer.cross_validate(
            model, mock_training_data, num_folds=2, param_grid=param_grid
        )
    ...
    def test_train(
        ...
        trained_model = LocusToGeneTrainer.train(mock_training_data, model)
