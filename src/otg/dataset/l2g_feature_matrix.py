...
class L2GFeatureMatrix(Dataset):
    ...
    _df: DataFrame = field(
        default_factory=partial(etl.spark.createDataFrame, [], schema=_schema)
    )
    ...
    def get_distance_features(
        ...
        return L2GFeatureMatrix(
            _df=self._df.unionByName(distance_features, allowMissingColumns=True)
        )

    def get_coloc_features(
        self: L2GFeatureMatrix, colocalisation: Colocalisation
