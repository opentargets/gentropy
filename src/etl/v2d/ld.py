...
def gnomad_ld(population: str, version: str) -> tuple[BlockMatrix, Table]:
    """LD matrix for reference population from gnomad.

    Args:
        population (str): reference population
        version (str): gnomad version

    Returns:
        Tuple[BlockMatrix, Table]: LD matrix and variant indices
    """
    bm_path = f"gs://gcp-public-data--gnomad/release/{version}/ld/gnomad.genomes.r{version}.{population}.common.adj.ld.bm"
    index_path = f"gs://gcp-public-data--gnomad/release/{version}/ld/gnomad.genomes.r{version}.{population}.common.adj.ld.variant_indices.ht/"
    bm, ld_index = get_ld_matrix(bm_path, index_path)
    return bm, ld_index


def get_ld_matrix(ld_path: str, ld_freq_path: str) -> tuple[BlockMatrix, Table]:
    """Load the LD matrix.

    Args:
        ld_path (str): Path to the LD matrix
        ld_freq_path (str): Path to LD allelic frequencies (used for indexing)

    Returns:
        Tuple[BlockMatrix, Table]: LD matrix and variant indices

    Raises:
        NotImplementedError: for now only gnomAD LD matrix is supported
    """
    if ld_path.startswith("gs://gcp-public-data--gnomad"):
        bm = BlockMatrix.read(ld_path)
        bm = bm + bm.T
        ld_freq = hl.read_table(ld_freq_path).naive_coalesce(400)
        return bm, ld_freq
    else:
        raise NotImplementedError


def liftover_loci(variant_index: Table) -> Table:
...
def interval_start(contig: Column, position: Column, ld_radius: int) -> Column:
...
def interval_stop(contig: Column, position: Column, ld_radius: int) -> Column:
...
def annotate_index_intervals(index: DataFrame, ld_radius: int) -> DataFrame:
...
def query_block_matrix(
    ...
    return (
        ...
        .to_spark()
        .filter(f.pow(entries.entry, f.lit(2)) >= min_r2)
    )
...
def lead_coordinates_in_ld(
    ...
    Args:
        leads_df (DataFrame): Lead variants from `annotate_index_intervals` output
...
def gnomad_toploci_ld_annotation(
    etl: ETLSession,
    lead_df: DataFrame,
    population: str,
    version: str,
    ...
    Args:
        ...
        lead_df (DataFrame): Dataframe containing lead variants
        population (str): gnomad reference population (e.g. "nfe")
        version (str): gnomad version
        ld_radius (int): basepairs around each position
        min_r2 (float): minimum r2 to keep
    ...
    bm, ld_index = gnomad_ld(population, version)
    # LD index liftovered to grch38
    ld_index = liftover_loci(ld_index)

    # LD index parsing from hail table to spark dataframe
    ldi = (
        ld_index.to_spark()
    ...
    # Leads (in population) including LD indexes starting and stopping the interval
    leads_with_idxs = lead_df.filter(f.col("pop") == population).join(
        annotate_index_intervals(ldi, ld_radius), on=["chrom", "pos", "ref", "alt"]
    )
    ...
    entries = query_block_matrix(bm, idxs, starts, stops, min_r2)
    ...
    lead_tag = (
        ...
        .join(
            ldi.select(
                "chrom",
                f.col("pos").alias("tag_pos"),
                f.col("ref").alias("tag_ref"),
                f.col("alt").alias("tag_alt"),
