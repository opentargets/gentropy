...
from etl.tissue_enrichment.tissue_enrichment import cheers, load_peaks
...
def main(cfg: DictConfig) -> None:
    ...
    summary_stats_credible_set = etl.spark.read.format("parquet").load(
        cfg.etl.tissue_enrichment.inputs.summary_stats_credible_set
    )

    pics_credible_set = etl.spark.read.format("parquet").load(
        cfg.etl.tissue_enrichment.inputs.pics_credible_set
    )
    tissue_annotations = load_peaks(
        etl.spark, cfg.etl.tissue_enrichment.inputs.tissue_annotations
    )

    summary_stats_enrichment = cheers(
        tissue_annotations,
        summary_stats_credible_set,
    )
