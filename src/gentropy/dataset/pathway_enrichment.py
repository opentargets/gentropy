...
class PathwayEnrichment(Dataset):
    ...
    | diseaseId        | pathway                                                                    | source  | normalisedEnrichmentScore | pValue   | pValueAdjusted |
    | ---------------- | -------------------------------------------------------------------------- | ------- | ------------------------- | -------- | -------------- |
    ...
    [`PathwayIndex`][gentropy.dataset.pathway_index.PathwayIndex], joined on `pathway`.
    """
    ...
    def from_gsea_catalogue(
        ...
        return cls(
            _df=session.spark.read.parquet(path).select(
                ...
                f.col("pathway").cast("string").alias("pathway"),
    ...
    def with_recomputed_adjusted_p_value(
        ...
        by_p_value = by_disease.orderBy(f.col("pValue").asc(), f.col("pathway").asc())
    ...
    def tested_pathways(self: PathwayEnrichment) -> DataFrame:
        ...
        Returns:
            DataFrame: Dataframe with a single `pathway` column.
        """
        return (
            self.df.filter(
