...
class LocusToGeneTrainer:
    ...
    def train(
        ...
        Args:
            ...
            train_df (pd.DataFrame | None): Pre-split training DataFrame. When provided together with ``test_df``, the hierarchical split is skipped and these DataFrames are used directly. Labels must already be numerically encoded (0/1 or float soft labels).
            test_df (pd.DataFrame | None): Pre-split held-out test DataFrame. Must be provided together with ``train_df``.
