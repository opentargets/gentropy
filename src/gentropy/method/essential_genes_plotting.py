"""Essential genes plotting functionality for disease group analysis."""
...
class EssentialGenesPlotter:
    ...
    def __init__(self, df: pd.DataFrame) -> None:
        ...
        Args:
            df: DataFrame containing essential genes data with uniqueDiseases column
                and essential gene score columns.
        """
        self.df = df.copy()
        self._validate_data()
    ...
    def _identify_essential_columns(self) -> None:
        ...
        # Look for columns that likely contain essential gene scores
        potential_columns = [
        ...
        # If no obvious essential gene columns found, use all numeric columns except identifiers
        ...
        self.essential_columns = potential_columns[:8]  # Limit to 8 as requested
    ...
    def _add_statistical_test(self, ax: plt.Axes, group_data: list[pd.Series]) -> None:
        ...
        if len(group_data) == 2 and len(group_data[0]) > 0 and len(group_data[1]) > 0:
            try:
                ...
                # If statistical test fails, just skip it
                pass
    ...
    def create_correlation_heatmap(self, figsize: tuple[int, int] = (12, 10)) -> None:
        """Create correlation heatmap for essential gene scores and disease associations.
        
        Args:
            figsize: Figure size as (width, height)
        """
        # Prepare correlation data
        corr_data = self.df[self.essential_columns + ['uniqueDiseases']].corr()
        ...
        disease_corr = corr_data['uniqueDiseases'].drop('uniqueDiseases').sort_values(
    ...
    def create_sample_data(n_genes: int = 1000) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with simulated essential gene data
        """
        ...
        # Create disease count data with realistic distribution
